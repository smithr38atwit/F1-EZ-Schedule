...
url = f"https://www.formula1.com/en/racing/{dt.today().year}.html"
...
def calendar() -> list[Race]:
    ...
    link_elements = soup.select(
        "a[data-roundtext]"
    )  # use CSS selector to find all link elements with attribute 'data-roundtext'
    ...
    for event in time_table.children:
        if type(event) is bs4.element.Tag:
            ...
            start_time = dt.strptime(
                event["data-start-time"] + event["data-gmt-offset"], "%Y-%m-%dT%H:%M:%S%z"
            )  # ex) '2023-03-04T18:00:00' + '+03:00'
